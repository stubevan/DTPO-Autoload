...
def check_method(pattern_spec) :
    ...
    entry = (
        ('value', 'database', 'Database Reset'),
        ('value', 'group', 'Group Reset'),
        ('value', 'tag', 'New Tag'),
        ('value', 'description', 'New Description'),
        ('pattern', 'string1_pattern', ('Pattern 3 key', 'Pattern 3 value', 3)),
        ('pattern', 'string2_pattern', (
            'Pattern2 2 key', 'Pattern2 2 value', 2)),
        ('pattern', 'date_pattern', ())
    )
    file_patterns.append(entry)

    entry = (
        ('value', 'database', 'Database Reset 2'),
        ('value', 'group', 'Group Reset 2'),
        ('value', 'tag', 'New Tag'),
        ('value', 'description', 'New Description'),
        ('pattern', 'string1_pattern', ('Pattern 3 key', 'Pattern 3 value', 3)),
        ('pattern', 'string2_pattern', (
        ...
        for check_details in details :
            ...
            if check_details[0] == 'value' :
                ...
                if not eval(check_string) :
                    print "Pattern check - bad entry in pattern {0} " \
                        ...
                        .format(counter + 1, check_parameter,
            ...
            #    Now check the patterns
                ...
                if len(check_value) == 0 :
                    ...
                    if error_check :
                        print "Pattern check - bad entry in pattern {0} " \
                            "for attribute {1}, {2}".format(counter + 1,
                                                            check_parameter,
                                                            error_check)
                        return False
        counter = counter + 1
